...
isSkipNeeded player fromField roll = any (fieldOccupiedBySelf player) fieldsBetween
    where
        ...
        numbersBetween = map show $ coreList [(fromFieldNr - roll) .. fromFieldNr] -- doesn't matter if the higher index goes last, all we want are the fields between
        fieldsBetween = map (createGoalCellNumber player) numbersBetween
...
makeEnterGoalMove player fromField roll = createGoalCellNumber player targetField
    where
        finalFieldNr = toInt $ finalField player
...
makeGoalMove player fromField roll = createGoalCellNumber player targetField
    where
        fromFieldNr = extractGoalCellNumber fromField
